 ...
 */
    ...
    init(manager: ActionManager, sender: Any, identifier: String, parameters: [String] = [], info: ActionInfo = ActionInfo()) {
...
extension ActionContext {
    ...
    public func notify(for action: String, stage: ActionNotificationStage, key: String = ActionContext.notificationKey) {
        info.forEach(key: key) { (notification: ActionNotification) in
            if (notification.action == "") || (notification.action == action) {
